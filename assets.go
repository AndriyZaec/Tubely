...
func processVideoFromStart(filepath string) (string, error) {
	processingFilepath := filepath + ".processing"
	cmd := exec.Command("ffmpeg", "-i", filepath, "-c", "copy", "-movflags", "faststart", "-f", "mp4", processingFilepath)
